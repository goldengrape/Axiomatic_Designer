...
我已收到并仔细研究了《评审意见：关于<L1-DP2 词汇服务 API 接口规范 (V1.0)>》。评审结论为“驳回，需强制性修订”。

我将严格依据评审意见中列出的两项强制性修订指令和两项建议性意见，对该 API 规范进行修订，以解决其中存在的关键模糊性与不一致性。修订的核心目标是：明确服务的部署假设，统一写入操作的响应行为，并提升规范的自解释性。

以下是根据上述评审意见修订后的完整文档。

---

### **L1-DP2 词汇服务 API 接口规范 (V1.1)**

**状态:** 草稿 (Draft)

**版本说明:** 本文档 (V1.1) 是对 V1.0 版本的重大修订，旨在响应并解决《评审意见：关于<L1-DP2 词汇服务 API 接口规范 (V1.0)>》中指出的设计缺陷。核心变更包括：
1.  **明确资源范围与部署假设 (指令1):** 在“API 通用约定”中增加了“资源范围与部署假设”一节，明确声明本服务被设计为单租户模式，消除了原设计中关于多租户能力的关键模糊性。
2.  **统一管理端点响应体 (指令2):** 重构了 `PUT /api/v1/lexicon` 端点的成功响应。对于资源更新，现返回 `204 No Content`；对于资源创建，现返回一个轻量级的 `LexiconSummary` 对象。这与系统内其他服务 (`L1-DP1`) 的行为模式保持一致，并遵循了 RESTful 最佳实践。
3.  **增加设计理由说明 (建议1):** 在“查询接口定义”部分增加了引言，阐明了同时提供全量获取和细粒度查询两种方式的设计意图，提升了规范的自解释性。
...
*   **成功响应:**
    ...
    *   **状态码:** `204 No Content` (当现有资源被成功替换时)
        *   **响应体:** 空。
*   **错误响应:**
...
# 场景 2b: 如果是替换旧文件，成功时返回 204 No Content，无响应体
